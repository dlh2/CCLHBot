...
MongoClient.connect(privatedata.url, function (err, db) {
	if (err) bot.sendMessage(msg.chat.id, "No se ha podido conectar a la base de datos");
		...
		//Iniciamos el Bot
		var game = new Cclhbot(bot, db, emoji);
		//////////////////////////////EVENTOS//////////////////////////////
			...
			if (msg.chat.type != "private") {
				...
				game.count('games', {room_id: msg.chat.id}, msg.chat.id, function(count_games) {
					...
					if (!count_games) {
						...
						game.count('players', {user_id: msg.from.id}, msg.chat.id, function(count_players){
		...
		bot.onText(/^\/votedelete(?:@cclhbot)?/i, function (msg, match) {
			...
			if (msg.chat.type != "private") {
				...
				game.find('games', {room_id: msg.chat.id}, msg.chat.id, function(r_game) {
					if (r_game.length) {
						game.find('players', {user_id: msg.from.id, game_id: r_game[0].game_id}, msg.chat.id, function(r_players){
							if (r_players.length){
								if (parseInt(r_players[0].vote_delete) == 0){
									if ((parseInt(r_game[0].vote_delete)+1) < (r_game[0].n_players/2)){ //ToDo?: usar los jugadores que hay en lugar del total
										game.update('games', {game_id: r_game[0].game_id}, {"vote_delete": (parseInt(r_game[0].vote_delete)+1)}, msg.chat.id, function (){
											game.update('players', {user_id: msg.from.id}, {"vote_delete": 1}, msg.chat.id, function (){
				...
				game.find('games', {room_id: msg.chat.id}, msg.chat.id, function(r_game) {
					if (r_game.length) {
						if (parseInt(r_game[0].currentblack)){
							game.count('cardsxround', {game_id: r_game[0].game_id}, msg.chat.id, function (count_cards){
								if (count_cards == r_game[0].n_players){
									game.sumax('cardsxround', 'votes', {game_id: r_game[0].game_id}, msg.chat.id, function(sum) {
			...
			if (msg.chat.type != "private") {
				...
				game.find('games', {room_id: msg.chat.id}, msg.chat.id, function(r_game) {
					if (r_game.length) {
						if (parseInt(r_game[0].currentblack)){
							game.find('players', {game_id: r_game[0].game_id}, msg.chat.id, function(r_players){
								game.find('cardsxround', {game_id: r_game[0].game_id}, msg.chat.id, function (r_cards){
		...
		bot.onText(/^\/join(?:@cclhbot)?\s(.*)/i, function (msg, match) {
			...
			if (msg.chat.type == "private") {
				...
				game.find('games', {game_id: match[1]}, msg.chat.id, function(r_game) {
					if (r_game.length) {
						if (!parseInt(r_game[0].currentblack)){
							game.count('players', {game_id: r_game[0].game_id}, msg.chat.id, function(count_players){
								if (count_players < r_game[0].n_players){
									game.count('players', {user_id: msg.from.id}, msg.chat.id, function(player){
										if (!player){
											//Obtenemos el nombre de usuario del creador
											var name = game.getUsername(msg);
											game.insert('players', {player_id: count_players+1, game_id: r_game[0].game_id, user_id: msg.from.id, username: name, points: 0, vote_delete: 0}, msg.chat.id, function(){
												//Y se le notifica por privado
												bot.sendMessage(msg.from.id, "Te has unido a la partida.");
										...
										if (n_cards >= r_game[0].n_players-1){
											game.find('cardsxround', {card_id: parseInt(match[1])}, msg.chat.id, function (r_cards){
												if (r_cards.length){
													game.find('players', {game_id: r_game[0].game_id}, msg.chat.id, function(r_players){
														if (r_players.length){
															game.find('players', {user_id: r_cards[0].user_id}, msg.chat.id, function(player){
																if (player.length){
																	var opts = {
																	...
																	if (r_game[0].type=="dictadura"){
																		...
																		game.find('players', {player_id: r_game[0].dictator_id}, msg.chat.id, function(dictator){
																			...
																			game.sumax('cardsxround', 'votes', {game_id: r_game[0].game_id}, msg.chat.id, function(cxr){
																				if (cxr[0].sum == r_game[0].n_players){
																					game.sortFind('cardsxround', {game_id: r_game[0].game_id}, {"votes": -1}, 1, msg.chat.id, function (card){
																						game.find('players', {user_id: card[0].user_id}, msg.chat.id, function(player){
																							bot.sendMessage(player[0].user_id, "Has ganado la ronda con tu carta: \n"+card[0].card_text+"\n Tienes "+(player[0].points+1)+" puntos.");
																							bot.sendMessage(r_game[0].room_id, player[0].username+" ha ganado la ronda con su carta: \n"+card[0].card_text+"\n Tiene "+(player[0].points+1)+" puntos.");
																							game.roundWinner(player[0], r_game, r_players, r_game[0].room_id);
																						});
															...
															game.insert('cardsxround', {card_id: n_cards.length+1, game_id: r_game[0].game_id, user_id: msg.from.id, card_text:match[2], votes: 0}, msg.chat.id, function(){
																...
																game.remove('wcardsxgame', {cxpxg_id: parseInt(match[1]), game_id: r_game[0].game_id, player_id: r_player[0].player_id}, msg.chat.id, function (){
																	...
																	if ((r_game[0].type=="dictadura" && n_cards.length+1 < r_game[0].n_players-1) || (r_game[0].type=="clasico" && n_cards.length+1 < r_game[0].n_players-1) || (r_game[0].type=="democracia" && n_cards.length+1 < r_game[0].n_players)){}
																		...
																		game.limitFind('bcardsxgame', {cxg_id: (parseInt(r_game[0].currentblack)), game_id: r_game[0].game_id}, 1, msg.chat.id, function (bcard){
																			if (bcard.length){
																				setTimeout(function (){
							...
							if (r_player.length){
								if (r_game[0].creator_id != msg.from.id){
									if ((r_game[0].type == "clasico" && r_player[0].player_id != r_game[0].dictator_id) || r_game[0].type != "clasico"){
										game.count('cardsxround', {user_id: msg.from.id, game_id: r_game[0].game_id}, msg.chat.id, function(r_player){
											if (!r_player){
