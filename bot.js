...
bot.on('text', function (msg) {
	...
	if (msg.chat.id < 0) {
		...
		if (msg.text.indexOf("/create") == 0){
			...
			//\s(dictadura|democracia)
			//numero_de_players-> \s([2-9])
			//numero_de_rondas-> \s([1-5])
			if (/^\/create(?:@cclhbot)?\s([1-9])/i.test(msg.text)) {
				...
				MongoClient.connect(privatedata.url, function (err, db) {
					if (err) bot.sendMessage(msg.chat.id, "No se ha podido conectar a la base de datos");
						...
						collection.find({room_id: msg.chat.id}).toArray(function (err, result) {
							if (err) {
								...
								collection.insertOne({game_id: base_36, room_id: msg.chat.id, creator_id: msg.from.id, dictator_id: msg.from.id, type: "dictadura"/*res[1]*/, num_participants: res[1]/*res[2]*/, num_cardstowin: 1/*res[3]*/, whitecards: shuffle(whitecards.list), blackcards: shuffle(blackcards.list), currentblack: 0}, function (err, result) {
									if (err) {
															...
															if (err) {
																...
																if (results.result.ok == 1){
																	...
																	for (i = 0; i < result2.length; i++){
																		var array = [];
																		for (j = 0; j < 6;j+=2){
																			array.push(["/"+j+" "+result2[i].cards[j], "/"+(j+1)+" "+result2[i].cards[j+1]]);
																		}
																		var opts = {
		...
		//Si el parametro es /join pedimos al usuario que lo envie por privado
			...
			bot.sendMessage(msg.chat.id, "Versión 0.1. Creador por @themarioga");
		}
										...
										if (players < result[0].num_participants){
											...
											if (!playergame){
												...
												if (err) {
													...
													db.collection('playersxgame').insertOne({player_id: players, game_id: res[1], user_id: msg.from.id, username: name, points: 0, cards:result[0].whitecards.slice(result2.length*45, result2.length*45+45)}, function (err, result3) {
														if (err) {
															bot.sendMessage(msg.chat.id, "Se ha producido un error al insertar en la tabla.");
															console.log(err);
														} else {
													...
													db.collection('playersxgame').find({user_id: result3[0].user_id}).toArray(function (err, result4) {
														if (err) {
															...
															if (result2[0].type=="dictadura"){
																...
																if (result3[0].user_id == result2[0].creator_id){
																			...
																			db.collection('playersxgame').updateOne({user_id: msg.from.id}, {$set: { "cards": result[0].cards}}, function(err, result5) {
																				if (err) {
																					...
																					if (result5.result.ok == 1) {
																						...
																						if (result3.length+1 < result2[0].num_participants) console.log();
																						else if (result3.length+1 == result2[0].num_participants) { //Si eres el ultimo en votar
